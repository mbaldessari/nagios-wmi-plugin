...
public class WindowsHealth {
	private static String helpmessage = 
		"Tool for:\n" +
		"  - monitoring free disk space, cpu and memory usage\n" +
		"  - finding all not started services which should be running\n" +
	...
	public WindowsHealth(String address, String domain, 
		...
		service = dispatch.callMethodA("ConnectServer",new Object[]{new JIString(address),JIVariant.OPTIONAL_PARAM(),JIVariant.OPTIONAL_PARAM(),JIVariant.OPTIONAL_PARAM(),
	...
	public LinkedList<IJIDispatch> getDiskDrives() throws JIException {
		...
		JIVariant results[] = service_dispatch.callMethodA("ExecQuery", new Object[]{new JIString("select * from Win32_LogicalDisk where DriveType = 3"), JIVariant.OPTIONAL_PARAM(), JIVariant.OPTIONAL_PARAM(),JIVariant.OPTIONAL_PARAM()});
	...
	public long getTotalMemorySize() throws JIException {
		...
		JIVariant results[] = service_dispatch.callMethodA("ExecQuery", new Object[]{new JIString("select * from Win32_OperatingSystem"), JIVariant.OPTIONAL_PARAM(), JIVariant.OPTIONAL_PARAM(),JIVariant.OPTIONAL_PARAM()});
		IJIDispatch wbemObjectSet_dispatch = (IJIDispatch)JIObjectFactory.narrowObject((results[0]).getObjectAsComObject());
		JIVariant variant = wbemObjectSet_dispatch.get("_NewEnum");
		IJIComObject object2 = variant.getObjectAsComObject();
	...
	public long getFreeMemorySpace() throws JIException {
		...
		JIVariant results[] = service_dispatch.callMethodA("ExecQuery", new Object[]{new JIString("select * from Win32_PerfRawData_PerfOS_Memory"), JIVariant.OPTIONAL_PARAM(), JIVariant.OPTIONAL_PARAM(),JIVariant.OPTIONAL_PARAM()});
		IJIDispatch wbemObjectSet_dispatch = (IJIDispatch)JIObjectFactory.narrowObject((results[0]).getObjectAsComObject());
		JIVariant variant = wbemObjectSet_dispatch.get("_NewEnum");
		IJIComObject object2 = variant.getObjectAsComObject();
	...
	public int getCPUUsage() throws JIException {
		...
		try {
			while (true) {
				JIVariant results[] = service_dispatch.callMethodA("Get", new Object[]{new JIString("Win32_Processor.DeviceID='CPU" + i + "'"), new Integer(0), JIVariant.OPTIONAL_PARAM()});
	...
	public LinkedList<IJIDispatch> getServices() throws JIException {
		...
		JIVariant results[] = service_dispatch.callMethodA("ExecQuery", new Object[]{new JIString("select * from Win32_Service where StartMode = 'Auto' and Started = FALSE"), JIVariant.OPTIONAL_PARAM(), JIVariant.OPTIONAL_PARAM(),JIVariant.OPTIONAL_PARAM()});
